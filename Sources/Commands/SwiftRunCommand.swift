...
public struct SwiftRunCommand: AsyncSwiftCommand {
    ...
    private func closeFdsAbove(_ from: Int32) {
        ...
        closefrom(from)
        #elseif os(Linux)
        // Use a closefrom(2)-like wrapper that uses close_range(2), or on
        // unsupported hosts has some faster fallbacks than getdtablesize + close().
        closeFrom(from)
        #else
        #if os(Android)
    ...
    #if os(Linux)
    // Small closefrom(2)-like wrapper for Linux. This uses close_range(2) under the hood if available
    // and if that fails will fall back to reading from /proc/self/fd.
    private func closeFrom(_ from: Int32) {
        let ret = _close_range(UInt32(from), UInt32.max, 0)
        if ret == -1 {
            closeFromFallback(from)
        }
    }

    private func closeFromFallback(_ from: Int32) {
        do {
            let entries = try FileManager.default.contentsOfDirectory(atPath: "/proc/self/fd")
            for entry in entries {
                if let fd = Int32(entry), fd >= from {
                    close(fd)
                }
            }
        } catch {
            // If reading proc also didn't work, let's just getdtablesize.
            let number_fds = getdtablesize()
            for i in from..<number_fds {
                close(i)
            }
